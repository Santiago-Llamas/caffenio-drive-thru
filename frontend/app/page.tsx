...
export default function Home() {
  ...
  if (pantalla === 'bienvenida') {
    return (
      <div className="flex items-center justify-center h-screen w-full bg-[#f5f5e9] bg-gradient-to-br from-[#efefe0] to-[#f7e7e7]">
        <div className="w-full max-w-[1280px] h-screen md:h-[800px] flex flex-col items-center justify-center py-8 md:py-16 px-4 md:px-12 relative">
          <main className="flex flex-col items-center justify-center flex-grow space-y-8 md:space-y-20 w-full">
            <button
              onClick={() => setPantalla('identificacion')}
              className="relative group outline-none"
            >
              <div className="w-48 h-48 md:w-96 md:h-96 rounded-full bg-white flex flex-col items-center justify-center text-center p-6 md:p-10 border-4 md:border-[16px] border-[#ea2a33] shadow-[0_0_80px_rgba(234,42,51,0.15),0_20px_40px_rgba(109,54,40,0.1)] transform transition-all active:scale-95 hover:scale-105">
                <span className="material-symbols-outlined text-[#ea2a33] text-5xl md:text-9xl mb-2 md:mb-4">touch_app</span>
                <span className="text-[#0A0A0A] text-xl md:text-4xl font-extrabold leading-tight px-2 md:px-4">
                  Presiona para iniciar
                </span>
              </div>
            </button>
            <div className="text-center space-y-2 md:space-y-4">
              <h1 className="text-4xl md:text-8xl font-extrabold text-[#6d3628] tracking-tight">¡Bienvenido!</h1>
              <p className="text-xl md:text-4xl font-medium text-[#6d3628]/80 tracking-wide">¿Qué se te antoja hoy?</p>
            </div>
          </main>
          <footer className="w-full flex justify-center mt-auto">
            <div className="flex gap-8 md:gap-16 text-xs md:text-sm font-bold uppercase tracking-[0.3em] text-[#6d3628]/40">
  ...
  if (pantalla === 'identificacion') {
    ...
    return (
      <div className="flex flex-col md:flex-row h-screen w-full bg-gradient-to-br from-red-50 to-slate-100 overflow-hidden">
        <main className="flex-1 flex flex-col p-4 md:p-16 bg-white/40 backdrop-blur-sm overflow-hidden relative">
          <div className="absolute top-0 left-0 w-48 md:w-96 h-48 md:h-96 bg-red-500/5 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>
          <div className="absolute bottom-0 right-0 w-32 md:w-80 h-32 md:h-80 bg-red-600/5 rounded-full blur-3xl translate-x-1/2 translate-y-1/2 pointer-events-none"></div>
          
          <div className="relative z-10 mb-6 md:mb-12">
            <h1 className="text-4xl md:text-7xl font-black tracking-tighter text-slate-900 mb-2">
          ...
          <div className="mt-6 md:mt-12 flex flex-col md:flex-row justify-between items-center relative z-10 gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 md:w-12 h-10 md:h-12 items-center justify-center">
                <img src="/logo.png" alt="Logo" className="w-full h-full object-cover" />
              </div>
              <span className="font-black text-lg md:text-2xl tracking-tighter uppercase italic text-slate-800">Caffenio</span>
            </div>
            <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6 text-center md:text-right">
              <div className="flex flex-col text-xs md:text-lg">
                <div className="font-bold text-slate-400">Punto de Venta: DRIVE-THRU TSM-1</div>
                <div>
                  <h6 className="font-bold text-slate-900">Developed by CODEX</h6>
                </div>
              </div>
              <div className="flex gap-2">
                <div className="w-2 md:w-3 h-2 md:h-3 rounded-full bg-green-500 animate-pulse"></div>
                <div className="w-2 md:w-3 h-2 md:h-3 rounded-full bg-slate-300"></div>
                <div className="w-2 md:w-3 h-2 md:h-3 rounded-full bg-slate-300"></div>
              </div>
            </div>
          </div>
  ...
  if (pantalla === 'favoritos') {
    ...
    return (
      <div className="flex flex-col md:flex-row h-screen w-full bg-[#f8f6f6] overflow-hidden">
        <main className="flex-1 overflow-y-auto bg-slate-50 relative pb-20">
          <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md px-4 md:px-8 py-3 md:py-4 border-b border-slate-200/60 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
            <button
              onClick={() => setPantalla('identificacion')}
              className="bg-white border border-gray-100 px-4 md:px-6 py-2 rounded-full text-slate-600 flex items-center space-x-2 active:bg-gray-50 transition-colors shadow-sm text-sm md:text-base"
            >
            ...
            <div className="bg-white px-4 md:px-6 py-2 md:py-3 rounded-full flex items-center space-x-3 border border-gray-100 shadow-sm text-xs md:text-base">
              <span className="material-icons-round text-slate-400 text-sm md:text-base">account_circle</span>
              <span className="text-[#6d3628] font-bold">
          ...
          <div className="p-4 md:p-8">
            <div className="text-center mb-6 md:mb-8">
              <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 tracking-tight">Tus Favoritos</h1>
              <p className="text-sm md:text-lg text-slate-500 mt-1">Selecciona tu bebida favorita para comenzar</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {favoritosMock.map((product) => (
                <div
                  key={product.id}
                  className="bg-white rounded-[40px] p-4 md:p-6 shadow-lg flex flex-col items-center justify-center border border-transparent hover:border-slate-200 transition-all group relative"
                >
                  <div className="relative w-32 md:w-40 h-32 md:h-40 mb-3 flex items-center justify-center">
                    <img
                      src={product.imagen}
                      alt={product.nombre}
                  ...
                  <div className="text-center mb-4">
                    <h3 className="text-lg md:text-xl text-slate-900 font-bold leading-tight">{product.nombre}</h3>
                    <p className="text-slate-500 text-sm md:text-base mt-1 tracking-wide font-bold">
                      ${product.precio.toFixed(2)}
                    </p>
                  </div>
                  <button
                    onClick={() => agregarAlCarrito(product, {})}
                    className="bg-[#ea2a33] text-white w-10 md:w-12 h-10 md:h-12 rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-all"
                  >
                    <span className="material-icons-round text-xl md:text-2xl font-bold">add</span>
                  </button>
                  <div className="absolute top-4 md:top-6 right-4 md:right-6 text-slate-200 group-hover:text-red-100 transition-colors">
                    <span className="material-icons-round text-xl md:text-2xl">favorite</span>
                  </div>
                </div>
              ))}
            </div>

            <div className="mt-8 md:mt-12 flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-12">
              <button
                onClick={() => setPantalla('menu')}
                className="flex flex-col items-center group"
              >
                <div className="bg-white w-12 md:w-14 h-12 md:h-14 rounded-2xl flex items-center justify-center shadow-lg mb-2 border border-slate-100 group-hover:border-slate-200 transition-all active:scale-95">
                  <span className="material-icons-round text-lg md:text-xl text-slate-400 group-hover:text-[#ea2a33]">add_circle_outline</span>
                </div>
                <span className="text-slate-500 text-xs md:text-sm font-bold">Explorar Menú</span>
              </button>
              <button
              ...
              >
                <div className="bg-white w-12 md:w-14 h-12 md:h-14 rounded-2xl flex items-center justify-center shadow-lg mb-2 border border-slate-100 group-hover:border-slate-200 transition-all active:scale-95">
                  <span className="material-icons-round text-lg md:text-xl text-slate-400 group-hover:text-[#ea2a33]">loyalty</span>
                </div>
                <span className="text-slate-500 text-xs md:text-sm font-bold">Promociones</span>
              </button>
            </div>
          </div>
        </main>

        <aside className="w-full md:w-80 bg-white border-t md:border-t-0 md:border-l border-slate-200 flex flex-col shadow-xl z-20 shrink-0 max-h-[40vh] md:max-h-none">
          <Carrito
            items={carrito}
